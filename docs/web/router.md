...
在 [src/router/routes/modules](https://github.com/elonehoo/tl-serve-list/tree/main/web/src/router/routes/modules) 内的 `.ts` 文件会被视为一个路由模块。
...
这里的 `icon` 配置，会同步到 菜单（icon 的值可以查看[此处](/components/icon)）。

## 新增路由

### 如何新增一个路由模块

1. 在 [src/router/routes/modules](https://github.com/elonehoo/tl-serve-list/tree/main/web/src/router/routes/modules) 内新增一个模块文件。
...
此时路由已添加完成，不需要手动引入，放在 [src/router/routes/modules](https://github.com/elonehoo/tl-serve-list/tree/main/web/src/router/routes/modules) 内的文件会自动被加载。
...
> [src/views/sys/redirect/index.vue](https://github.com/elonehoo/tl-serve-list/blob/main/web/src/views/sys/redirect/index.vue)
...
1. 在 [src/settings/projectSetting.ts](https://github.com/elonehoo/tl-serve-list/blob/main/web/src/settings/projectSetting.ts) 内将 `openKeepAlive` 设置为 `true`
2. 路由设置 `name`，且**不能重复**
...
可以将[pageEnum.ts](https://github.com/elonehoo/tl-serve-list/blob/main/web/src/enums/pageEnum.ts)中的`BASE_HOME`更改为需要你想设置的首页即可
